...
class GameScene: SKScene, SKPhysicsContactDelegate{
    ...
    override func didMoveToView(view: SKView) {
        self.view?.makeToast(message: "Round 1")
        ...
        // skyline
        let skyTexture = SKTexture(imageNamed: "citynightbg")
        ...
        // setup our bird
        let birdTexture1 = SKTexture(imageNamed: "Crab.png")
        birdTexture1.filteringMode = .Nearest

        let birdTexture2 = SKTexture(imageNamed: "Crab.png")
        ...
        gameStatus.position = CGPoint( x: self.frame.midX, y: self.frame.size.height/2+50)
        gameStatus.zPosition = 100
        gameStatus.fontSize =  80
        gameStatus.fontColor =  SKColor.whiteColor()
            ...
            if ( contact.bodyA.categoryBitMask & scoreCategory ) == scoreCategory || ( contact.bodyB.categoryBitMask & scoreCategory ) == scoreCategory {
                ...
                if (score == 15)
                {
                   self.removeActionForKey("flash")
                    ...
                    self.view?.makeToast(message: "Congratulations. You have completed The First round. ")

                    //moving to level two
                    var gameStartScene = SecondRound(size: self.frame.size)
                ...
                self.runAction(SKAction.sequence([SKAction.repeatAction(SKAction.sequence([SKAction.runBlock({
                    ...
                    var scoreString: String =  self.scoreLabelNode.text!.stringByReplacingOccurrencesOfString("Score : ", withString: "", options: [], range: nil)
                    GameOverController().getScoreDisplayData("Round :  1", Score: scoreString)
